    ...
    <>
      <div ref={navRef} className="fixed w-full z-50 transition-all duration-500">
        ...
        <nav 
          className={`w-full transition-all duration-500 backdrop-blur-md ${
            scrolled 
              ? 'bg-white/95 shadow-[0_8px_30px_rgb(0,0,0,0.12)] py-2 border-b border-green-100' 
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex-shrink-0 group">
                <Link href="/" className="flex items-center">
                  ...
                  <div className={`relative h-12 w-12 rounded-xl overflow-hidden flex items-center justify-center mr-3 shadow-lg group-hover:shadow-green-300/50 transition-all duration-300 transform group-hover:scale-105 ${
                    scrolled ? 'bg-white' : 'bg-white/90'
                  }`}>
                    {/* Using the actual logo image */}
                    <Image 
                      src="/images/logo.jpg" 
                      alt="RichyElectricals Ghana Logo" 
                      layout="fill"
                      objectFit="cover"
                    />
                  </div>
              ...
              <div className="hidden md:flex md:items-center">
                <div className="ml-10 flex items-baseline space-x-1">
                  ...
                  <NavLink href="/projects" text="Ghana Projects" scrolled={scrolled} />
                  <Link 
                    href="/contact" 
                    className="relative overflow-hidden bg-gradient-to-r from-green-500 to-green-600 text-white px-5 py-2 rounded-full font-medium hover:shadow-lg hover:shadow-green-300/50 transition-all duration-300 ml-3 transform hover:-translate-y-0.5 hover:scale-105"
                  >
      ...
      >
        <div className="px-4 pt-6 pb-20 space-y-4 flex flex-col items-center max-h-[calc(100vh-120px)] overflow-y-auto">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center mb-6 shadow-lg shadow-green-300/30">
            <span className="text-white font-bold text-xl">REG</span>
          </div>
          ...
          <MobileNavLink href="/projects" text="Ghana Projects" onClick={toggleMenu} />
          <MobileNavLink href="/faq" text="FAQ" onClick={toggleMenu} />
          <a 
            href="https://richyelectricals.co.uk" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="w-full max-w-xs text-center text-gray-800 hover:text-green-500 px-4 py-4 rounded-lg text-lg font-medium border-b border-gray-100 transition-all duration-300 hover:bg-green-50"
            onClick={toggleMenu}
          >
            RichyElectricals UK
          </a>
          <Link 
            href="/contact" 
            className="w-full max-w-xs text-center bg-gradient-to-r from-green-500 to-green-600 text-white px-5 py-3 rounded-full font-medium shadow-lg shadow-green-300/20 transition-all duration-300 mt-6 hover:shadow-green-300/40 transform hover:scale-105"
            onClick={toggleMenu}
          >
            Contact Us
          </Link>
...
const NavLink = ({ href, text, scrolled }) => (
  <Link 
    ...
    className={`relative px-4 py-2 rounded-full font-medium group transition-all duration-300 ${
      scrolled ? 'text-gray-800 hover:text-green-600' : 'text-gray-700 hover:text-green-500'
    }`}
  >
    {text}
    <span className="absolute bottom-0 left-1/2 w-0 h-0.5 bg-green-500 group-hover:w-4/5 group-hover:left-1/10 transition-all duration-300"></span>
  </Link>
);
